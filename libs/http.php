...
function getHeadersFromUrl($url) {
	...
	if (!curl_errno($ch)) {
		$headers["status"] = curl_getinfo($ch, CURLINFO_HTTP_CODE);
	}
	curl_close($ch);
	return $headers;
}

function pingService($host, $port = 443, $timeout = 1){
...
function getServiceIPListFromHost($host) {
    return gethostbynamel($host);
}

function getServiceIP($host, $port = 443) {
    $hostList = getServiceIPListFromHost($host);
    foreach ($hostList as $k => $h) {
        if (pingService($h, $port) == true) {
            return $h;
        }
    }
    return null;
